...
const ProjectCard = ({web, repo, title, desc, img}) => {
  ...
  return (
    ...
    variants={{
      visible: { opacity: 1 },
      hidden: { opacity: 0 }
    }} className=' lg:p-0 md:p-2 p-5 w-full flex'>
    <div className='proj rounded-2xl flex flex-col lg:flex-row flex-wrap lg:flex-nowrap justify-center items-center gap-4' >
        <motion.div initial={"hidden"} whileInView="visible" transition={{ duration: 0.8, ease: 'easeOut' }}
          variants={{
        ...
        {/* <a href={web} target="_blank" className=' lg:flex hidden justify-center items-center w-full'>
            ...
            <a href={web} target="_blank" className=' flex w-full p-5'>
            <div className="flex justify-center items-center w-full" >
                <img className='rounded-xl border border-slate-100' src={img} preload='metadata' style={{}}>
                </img>
            </div>
            </a>
            </motion.div>
      ...
      <button type="button" className=" card smbtn text-white lg:my-0 sm:my-2  hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 text-center mr-3 md:mr-0 dark:bg-blue-600 dark:hover: dark:focus:ring-blue-800">Visit Webiste</button>
      </a>
