...
export default function Achievements() {
  ...
  const achswiperRef = useRef();
  const [page, setPage] = useState(0);
  useEffect(() => {
    if (page <= 0) {
      document.getElementById("achpbtn").style.color = "#656565";
    } else {
      document.getElementById("achpbtn").style.color = "#ffffff";
    }
    if (page >= achievements.length - 1) {
      document.getElementById("achnbtn").style.color = "#656565";
    } else {
      document.getElementById("achnbtn").style.color = "#ffffff";
    }
  }, [page, achievements]);
  return (
    <div id='achievements' className=' flex w-full flex-col justify-center items-center min-h-screen'>
        <h1 className=' lg:primaryText lg:p-head projhead w-full text-center py-8'> Achievements</h1>
        ...
        <Swiper
          ...
          slidesPerView={1}
          onSlideChange={() => setPage(achswiperRef.current.activeIndex)}
          onSwiper={(swiper) => {
            achswiperRef.current = swiper;
          }}
          virtual
        >
          {achievements.map((element, index) => (
            <SwiperSlide key={element} virtualIndex={index}>
            <AchievementCard url={element.url} title={element.title} desc={element.desc} img={element.img} />
            </SwiperSlide>
          ))}
        </Swiper>
        ...
        <svg xmlns="http://www.w3.org/2000/svg" onClick={() => {
              if (page > 0)
                setPage(page - 1)
              achswiperRef.current.slidePrev()
            }} 
            id='achpbtn'
            fill='currentColor'
            stroke="currentColor"
...
<path d="M 25 2 C 12.308594 2 2 12.308594 2 25 C 2 37.691406 12.308594 48 25 48 C 37.691406 48 48 37.691406 48 25 C 48 12.308594 37.691406 2 25 2 Z M 25 4 C 36.609375 4 46 13.390625 46 25 C 46 36.609375 36.609375 46 25 46 C 13.390625 46 4 36.609375 4 25 C 4 13.390625 13.390625 4 25 4 Z M 27.875 14 C 27.652344 14.023438 27.441406 14.125 27.28125 14.28125 L 17.28125 24.28125 C 17.085938 24.46875 16.976563 24.730469 16.976563 25 C 16.976563 25.269531 17.085938 25.53125 17.28125 25.71875 L 27.28125 35.71875 C 27.679688 36.117188 28.320313 36.117188 28.71875 35.71875 C 29.117188 35.320313 29.117188 34.679688 28.71875 34.28125 L 19.4375 25 L 28.71875 15.71875 C 29.042969 15.417969 29.128906 14.941406 28.933594 14.546875 C 28.742188 14.148438 28.308594 13.929688 27.875 14 Z"></path>
          ...
          <div className='col-span-3 ' style={{ alignItems: 'center', alignContent: 'center', }}>
            <ProgressBar completed={page * (100 / (achievements.length - 1))} isLabelVisible={false} bgColor='#ffffff' baseBgColor='#0f0f0f' animateOnRender={true} height={5} />
          </div>
          <div className='col-span-1 flex justify-end items-end mx-2' style={{ placeItems: 'end' }}>
          <svg onClick={() => {
                if (page < achievements.length - 1)
                  setPage(page + 1)
                achswiperRef.current.slideNext()
              }}
              fill='currentColor'
              stroke="currentColor"
              className="h-16 w-16 scale"
              style={{ color: '#ffffff' }} id='achnbtn' xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 50 50">
